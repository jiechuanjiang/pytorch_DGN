...
class ReplayBuffer(object):
	...
	def __init__(self, buffer_size, obs_space, n_action, n_ant):
		...
		self.mask = np.zeros((self.buffer_size,self.n_ant,n_action))

	def getBatch(self, batch_size):

		index = np.random.choice(self.len, batch_size, replace=False)
		return self.obs[index], self.actions[index], self.rewards[index], self.next_obs[index],self.matrix[index],self.next_matrix[index],self.next_mask[index], self.dones[index], self.mask[index]

	def add(self, obs, action, reward, next_obs, matrix, next_matrix, next_mask, done, mask):
